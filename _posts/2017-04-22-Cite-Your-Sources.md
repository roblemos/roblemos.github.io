...
categories: blogging data-science journalism
---

_Digging into a data point that has minimal citation finds it is more three years older than expected and came from a completely different source._

Every week -- if not every day -- I come across reports that quote a data point with very little information about the source. Here is a recent one:
...
The data point is also in the press release. The first problem I have with it is that there is no citation nor link for the source of the data. The second problem I have is that -- and this took far more digging than is warranted -- the 10 to 20 percent number is **not even from Forrester Research**.
...
So, here's the rub: The Forrester report is not the source for the data point. The report cites a Sonatype report from 2015 [with a link](http://cdn2.hubspot.net/hubfs/1958393/White_Papers/2015_State_of_the_Software_Supply_Chain_Report-.pdf?t=1466775053631). (Kudos to Forrester Research for that.)

One problem, however, is that finding the number in the 33-page report is not easy. Searching for "10 to 20 percent" doesn't return anything, even searching through all the "10" entries does not work either. The reason is that the original data point is "an average of 106 components comprise 80-90% of
the total application," which can be found on page 22. (It was around this point that I decided this would make a good blog post.) The point is also mentioned on page 5.

The trail did not stop here, of course. Sonatype cited older research in both.

### 4. Sonatype report cites ongoing Application Health Check data (circa 2014)
